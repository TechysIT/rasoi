...
export const columns: ColumnDef<ExpenseData>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (paymentMethod === PaymentMethod.cash) methodColor = "text-green-500";
      if (paymentMethod === PaymentMethod.bank) methodColor = "text-blue-500";
      if (paymentMethod === PaymentMethod.card) methodColor = "text-purple-500";

      return (
        <div className={`capitalize font-medium ${methodColor}`}>
          {paymentMethod}
        </div>
      );
    },
...
export function ExpenseTable({ data }: { data: ExpenseData[] }) {
